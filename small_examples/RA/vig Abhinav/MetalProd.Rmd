...
2015 production value is set as a reference/index value of 100. These indexes are good proxies to track actual production growth (or decline) and are available for most classification of industries and services. A guide to add variables to the dictionary can be found here as well: [Adding a variable to our dictionary](https://moritzschwarz.org/aggregate.model/articles/new_variable_to_dict.html)
...
Industrial Emissions, and Emissions from Combustion. Since Metal Production is a significant contributor to the overall industrial production of Germany, a significant relationship could provide us with the effect our policy can have on overall Industrial Production. Similarly, for Industrial Emissions, the choice of using only `Metal_Prod` (and other non-prod indicators) is to observe the isolated effect of `Metal_Prod` on Emissions. HICP_Energy provides us a good proxy of energy prices which is believed to have a negative effect on emissions. The specification of model in this way allows us to choose a variety of combinations of variables and also check for whether the relationships we assume are statistically significant in this combination. Although, it is duly acknowledged that this way is vulnerable to Omitted Variable Bias much like other linear models, thus the outcome shouldn't be necessarily interpreted causally.
...
model <- run_model(specification = specification,
                   dictionary = dict_de,
                   inputdata_directory = here::here("data-raw/metal/"),
                   primary_source = "local",
                   save_to_disk = here::here("data-raw/metal/data_metal.xlsx"),
...
For this particular scenario, a three equation specification of the model has been used:
...
```
##### Choice of Variables
In this specification, I have modelled Industrial Emissions, Emissions from Combustion, and Household Consumption Expenditure. As is with the case with the previous model, we are interested with only the isolated effect of `HICP_Energy` on `EmiCO2Combustion`, we will interpret the results for only the second module. The other variables are chosen to observe other assumed relationships, if they exist in this combination of equation, which may influence the interaction of our dependent variable `HICP_Energy` with `EmiCO2Combustion`. For instance, in this combination, it can be said that there isn't a significant impact of `HICP_Energy` on Industrial Emissions and thus can be tested for other emission categories.
...
model2 <- run_model(specification = spec,
                   dictionary = new_dict,
                   inputdata_directory = here::here("data-raw/your_folder/"),
                   primary_source = "local",
                   save_to_disk = here::here("data-raw/your_folder/data_new.xlsx"),
                   saturation.tpval = 0.001,
                   max.ar = 2,
                   max.dl = 2,
                   quiet = FALSE,
...
Now, we can have a look at the model summary. The negative coefficient on HICP_Energy in the EmiCO2Combustion represents a negative relationship between prices and combustion emissions. This result follows the simple logical mechanism of higher prices, lower demand and thus lower use of energy, suggesting lower emissions derived from fossil based energy combustion.
